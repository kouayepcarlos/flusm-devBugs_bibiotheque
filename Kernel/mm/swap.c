...
static void __put_single_page(struct page *page)
{
	...
	if (test_bit(PG_usm, &page->flags)) {
		trace_printk(KERN_INFO "Nope #USM__PSP\n");
...
{
	if (test_bit(PG_usm, &page->flags)) {
		trace_printk(KERN_INFO "Nope #USM__PP\n");
		//usm_note_page(page);
		//return;
	}
	if (unlikely(is_zone_device_page(page))) {
		trace_printk("Nope & is..dev....\n");	//? && PG_usm..?
...
void put_pages_list(struct list_head *pages)
{
	...
	list_for_each_entry_safe(page, next, pages, lru) {
		if (test_bit(PG_usm, &page->flags)) {
			trace_printk(KERN_INFO "Nope #USM__PPL\n");
			//usm_note_page(page);
			//continue;
		}
	...
	void (*move_fn)(struct page *page, struct lruvec *lruvec))
{
	...
	for (i = 0; i < pagevec_count(pvec); i++) {
		...
		struct folio *folio;
		if (test_bit(PG_usm, &page->flags)) {
			trace_printk("pagevec_lru_move_fn...\n");
			//continue;
		}
...
static void pagevec_move_tail_fn(struct page *page, struct lruvec *lruvec)
{
	struct folio *folio = page_folio(page);
	if (test_bit(PG_usm, &page->flags)) {
		trace_printk("pagevec_move_tail_fn...\n");
		//return;
	}
...
static bool pagevec_add_and_need_flush(struct pagevec *pvec, struct page *page)
{
	bool ret = false;
	if (test_bit(PG_usm, &page->flags)) {
		trace_printk("pagevec_add_and_need_flush...\n");
...
{
	/* TODO get them pages and test one by one... | &folio->page */
...
static void lru_deactivate_file_fn(struct page *page, struct lruvec *lruvec)
{
	...
	//if (test_bit(PG_usm, &page->flags))										// TODO in fact, in this file, we could find a way not to use pte_access anymore.. if we note activations of __activate_page, for example....hmm, to further investigate
		//return;
...
void deactivate_file_folio(struct folio *folio)
{
	struct pagevec *pvec;
	if (test_bit(PG_usm, &(&folio->page)->flags)) {
		trace_printk(KERN_INFO "Nope #USM__deactivate'file'folio\n");
		return;
	}
	/*
...
void release_pages(struct page **pages, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		//if (test_bit(PG_usm, &page->flags) && (atomic_read(&page->_refcount)<=1)) {	/* &&(atomic_read(&(page)->_refcount)==1)*/
			//trace_printk("Could be released.. #release_pages\n");
			//usm_note_page(page);
			//continue;
		//}

		page = &folio->page;
		//if (test_bit(PG_usm, &page->flags) && (atomic_read(&page->_refcount)<=1)) {
			//trace_printk(KERN_INFO "Nope #USM__RPs (treating')\n");
			/*usmp = kmalloc(sizeof(*usmp), GFP_KERNEL);
			if (!usmp) {
				trace_printk(KERN_INFO "Not treated #USM__RPs (kmalloc, one lost)\n");
				continue;
			}
			...
			//trace_printk("Could be released.. #release_pages'2\n");
			//usm_note_page(page);
			//continue;
		//}
...
void __pagevec_lru_add(struct pagevec *pvec)
{
	...
	for (i = 0; i < pagevec_count(pvec); i++) {
		...
		if (test_bit(PG_usm, &(&folio->page)->flags)) {
	        ...
	        atomic_set(&(page)->_mapcount, 2);*/
			/*spin_unlock(&usmPagesLock);*/
			//usm_note_page(page);							Hmmm.... TODO HUGE
			trace_printk("Not added to LRU here.. #__pagevec_lru_add\n");
			continue;
		}
		lruvec = folio_lruvec_relock_irqsave(folio, lruvec, &flags);
