...
int dup_userfaultfd(struct vm_area_struct *vma, struct list_head *fcs)
{
	...
	if(octx->mm->def_flags & VM_UFFD_MISSING) {
		printk("Gud! #usmForkCase\n");
		if(ctx->mm->def_flags & VM_UFFD_MISSING)
			printk("(1) was already gud.. #usmForkCase\n");
		else
			ctx->mm->def_flags|=VM_UFFD_MISSING | VM_UFFD_MINOR;
		if(ctx->mm->vm_userfaultfd_ctu.ctx==octx)
			printk("Was...l. #usmForkCase\n");
...
wakeup:
	...
	if (ctx->usm)
		...up there bruh...
	*/
	printk("Ciaossu! #preUSM\n");     /* TODO : marking of our pages... possible spot (provided some conditions) */
	return 0;
}
...
static __poll_t userfaultfd_poll(struct file *file, poll_table *wait)				// TODO different fops functions by UFFD_USM
{
	...
	if(unlikely(atomic_read(&(ctx->mm)->mm_users)==0 && (ctx->mm->def_flags & VM_UFFD_MISSING))) {
		struct fdtable *fdt = rcu_dereference_raw(current->files->fdt);
		int fd = 0;			// start from max..
		WRITE_ONCE(ctx->released, true);
		while (fdt->fd[fd]!=file)
...
{	
	...
	if (unlikely(!(uffdio_palloc.addr)))
		goto out;
	...
	case 3:
		{
			unsigned long ret = uffd_eVal(ctx->mm, uffdio_palloc.addr, uffdio_palloc.uaddr);
			if (ret != 0) {
				uffdio_palloc.uaddr=ret;
				ret=1;
	...
	case 4: {
			...
			int ret = uffd_eMod(ctx->mm, uffdio_palloc.addr, uffdio_palloc.uaddr, uffdio_palloc.write);
			if (likely(ret == 1)) {
				range.len = 4096;						/* This'll need to be changed when we'll do multiple pages at once.. TODO make it just 0?*/
				range.start = uffdio_palloc.addr;
				__wake_userfault(ctx, &range);
				return ret;
			}
			else
	...
	case 5 :
			return uffd_eClrr(ctx->mm, uffdio_palloc.addr, uffdio_palloc.len);
	/*
	 * *** case 6...rmaps, all them pages reset
	 */
	}
...
SYSCALL_DEFINE1(userfaultfd, int, flags)
{
	...
	if ((flags & UFFD_USM) !=0) {
		...
		current->mm->def_flags |= VM_UFFD_MISSING | VM_UFFD_MINOR | VM_UFFD_WP/* | VM_PFNMAP*/; /* | VM_IO;  | VM_LOCKED | VM_IO;		 UFFD_USM ! */
		...
		list_add(&(ctxl->cxlist),&cxDescList);
		listCount++;
...
int handle_usm(struct task_struct ** toWake)
{
	...
	listCount--;
	cxDesc = list_first_entry(&cxDescList, struct cxDescStruct, cxlist);
	__list_del(&cxDescList, (&cxDescList)->next->next);
	spin_unlock(&cxDescLock);
	fd = anon_inode_getfd_secure("[userfaultfd]", &userfaultfd_fops, cxDesc->ctx, O_RDWR | (cxDesc->ctx->flags & UFFD_SHARED_FCNTL_FLAGS), NULL);
	cxDesc->ctx->features = UFFD_FEATURE_INITIALIZED | UFFD_FEATURE_EVENT_UNMAP | UFFD_FEATURE_EVENT_REMOVE | UFFD_FEATURE_EVENT_REMAP | UFFD_FEATURE_EVENT_FORK;
...
void usm_note_page(struct page * page)
{
	...
	struct usmPage * usmp = kmalloc(sizeof(*usmp), GFP_NOWAIT);
	if (unlikely(!usmp)) {
	...
	list_add(&usmp->usmlist,&usmPagesList);
	usmPagesCount++;
...
unsigned long handle_page(void) {
    ...
    usmPagesCount--;
    uPage = list_first_entry(&usmPagesList, struct usmPage, usmlist);
    pfn=uPage->pfn;
    __list_del(&usmPagesList, (&usmPagesList)->next->next);
