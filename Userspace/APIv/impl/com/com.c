...
void *usm_handle_evt_poll(void *wk_args)
{ // no need anymore of "poll" and opposed concepts.. they'll be in usm_check..
...
int usm_hints_ret_ev_userI_connection(struct usm_event *event)
{
    ...
    sscanf(shm_ptr, "%d", &pid);
    sscanf(shm_ptr, "%s", nom_processus);
    sscanf(shm_ptr, "%d", &taille);
    sscanf(shm_ptr, "%s", request);

    int index = find_gedit_index(procDescList, nom_processus);
    if (index != -1)
        {
            event->origin = index;
        }
    ...
    event->procName = (char*)malloc(sizeof(char)*30);
    memcpy(event->procName, nom_processus, sizeof(char)*30);
    event->taille = taille;
    event->type = HINTS;
    event->data=(char*)malloc(taille);
         
    memcpy(event->data,request,taille);
...
// int usm_uffd_subm_ev_zero_kernel(struct usm_event *event)
// {
//     usm_set_pfn_zero_kernel(event->origin, event->vaddr, event->paddr, 0);
// }
...
struct usm_channel_ops usm_cnl_hints_new_userI_ops = {
    .usm_retrieve_evt = usm_hints_ret_ev_userI_connection,
...
// struct usm_channel_ops usm_cnl_userfaultfd_ops_zero_kernel = {
//     .usm_retrieve_evt = usm_uffd_ret_ev,
//     .usm_submit_evt = usm_uffd_subm_ev_zero_kernel,
//     .usm_check = usm_poll_check};
