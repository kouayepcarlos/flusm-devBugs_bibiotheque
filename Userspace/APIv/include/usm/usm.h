...
static inline int usm_set_pfn(int process, unsigned long addr, unsigned long pfn,  int write, int channelType ) {				// channelType to be used later, when there will be other ways of doing things than ioctl
    struct uffdio_palloc uffdio_palloc= {.addr=addr,.uaddr=pfn,.opt=4, .write=write};
...
// static inline int usm_set_pfn_zero_kernel(int process, unsigned long addr, unsigned long pfn, int channelType) {				// channelType to be used later, when there will be other ways of doing things than ioctl
//     struct uffdio_palloc uffdio_palloc= {.addr=addr,.uaddr=pfn,.opt=6};
//     if (ioctl(process, UFFDIO_PALLOC, &uffdio_palloc) != 1) {
// #ifdef DEBUG
//         perror("\t\t[Mayday] Allocation failed!");
// #endif
...
// static inline int usm_set_pfn(int process, unsigned long addr, unsigned long pfn, int channelType,int write) {				// channelType to be used later, when there will be other ways of doing things than ioctl
